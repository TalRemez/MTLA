...
and QD-DETR. All COCO runs prompt the model with the full 80-class COCO vocabulary on every image
(open-vocabulary); see the note below.
...
> **COCO prompting note.** The COCO numbers here prompt the model with the **full 80-class COCO
> vocabulary** on every image, matching an open-vocabulary detection setting. An earlier InternVL
> detection recipe instead listed only the classes present in each image (a per-image oracle subset,
> ~3 classes/image), which makes the task easier and inflates AP — that recipe produced a higher
> InternVL AP but is not the open-vocabulary experiment described here, so this repo does not use
> it. Qwen3-VL-8B is the headline COCO detector: it is prompted with all 80 classes in every stage
> and its prediction counts match the open-vocabulary setting. Under the honest all-80 prompt the
> MTLA hallucination-AUROC is unchanged-to-better (InternVL 0.916); the difference is an **AP**
> effect from the oracle class list, not an MTLA-signal effect.
