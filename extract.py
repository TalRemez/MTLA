...
def worker(
    ...
    # A few skipped items (missing file, unparseable response) are tolerated, but a shard that
    # extracted NOTHING from a non-trivial number of records is systemic (wrong data dir, every
    # clip missing, a broken path scheme) — fail loud rather than write an empty shard silently.
    if not out and len(records) > 1:
        raise RuntimeError(
            f"[worker {rank}] extracted 0 of {len(records)} records; this is a systemic "
            f"failure (check the media paths / data dir), not a few missing examples."
        )
...
def extract_seed(cfg, seed: int) -> None:
    ...
    Args:
        cfg: the loaded run config (paths, extract GPUs).
...
def main() -> None:
    ...
    rate-limiting), auto-discovers the seed set from the predictions dir (there is no
    ``--n`` or ``--limit``: extract processes exactly what generate wrote), then calls
    ``extract_seed`` for each discovered seed.
    """
