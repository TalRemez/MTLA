...
from mtla.types import GTRegion, OverlapFn, Region
...
def parse_timestamp(s: str) -> float:
    ...
    Args:
        s: A timestamp, either ``"MM:SS(.fff)"`` or a plain ``"SSS(.fff)"`` seconds value.
...
def video_duration(video_path: str) -> float:
    """Duration of a video file in seconds, via decord.

    ``decord`` is imported lazily so ``import mtla.utils`` stays light on the CPU-only
    score path (which never touches video).

    Args:
        video_path: Path to the video file.

    Returns:
        Duration in seconds (``frame_count / average_fps``), or ``0.0`` if the average
        fps is not positive.
    """
