...
Typical use, starting from extracted attention records (see ``examples/`` for extraction):
    ...
    from mtla import mtla_score, svar_score, auroc_from_records

    auroc_mtla = auroc_from_records(records, stat="image_inside_sum")  # ours
    auroc_svar = auroc_from_records(records, stat="image_sum")         # baseline
"""
from .score import (
    ALL_LAYERS,
    DEFAULT_BAND,
