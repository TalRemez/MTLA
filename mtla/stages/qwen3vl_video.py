...
def worker(rank, gpu_id, items, out_dir, pred_dir, video_dir, svar_shift=False, seed=None,
    ...
    # mode="extract": read each item's saved `response` (from `responses`, keyed by vid/qid),
    #   run the monkeypatched forward, write attention .pt only. The vision preprocessing,
    #   token indexing and extraction below are IDENTICAL across modes (decoupled-parity).
    ...
    for cnt, item in enumerate(items):
        ...
        if mode == "extract":
            # decoupled: take the response produced by the generate stage (no re-generation)
            response = responses.get(item.get("qid"), responses.get(vid))
            if response is None:
                n_skipped += 1; continue
        else:
            try:
        ...
        if mode == "generate":
            # generation stage: record the prediction (incl. geometry the extract stage needs to
            # re-derive masks deterministically) and move on — no attention forward.
            preds_out.append({
...
def main():
    ...
    ap.add_argument("--mode", choices=["generate", "extract"], default="extract",
                    help="generate -> predictions.json only; extract -> attention .pt only "
                         "(reads predictions.json for the responses).")
    args = ap.parse_args()
    ...
    print(f"QVHighlights val queries: {len(items)}  mode={args.mode}")

    # extract mode: load responses produced by the generate stage, keyed by qid and vid.
    responses = {}
    if args.mode == "extract":
        pj = f"{args.pred_dir}/predictions.json"
        for p in json.load(open(pj)):
            if p.get("qid") is not None:
                responses[p["qid"]] = p["response"]
            responses[p["vid"]] = p["response"]
        # only score items we have a response for
        items = [it for it in items if it.get("qid") in responses or it.get("vid") in responses]
