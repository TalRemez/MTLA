...
Example (configs/coco_internvl.yaml)::
    ...
    model: internvl_image      # -> mtla.models.internvl.InternVLImageAdapter
...
(n_rollouts) and ``--limit`` (first N items; default: all) on generate.py, and ``--gpus`` on
generate.py / extract.py (defaults: 1 rollout, all visible GPUs, all items).
"""
...
def all_visible_gpus() -> list:
    ...
    Returns:
        ...
        ``[0]`` (``device_count`` returns 0, not raises, when CUDA is unavailable).
    """
...
class StageCfg:
    ...
    / ``greedy_seed0`` / ``max_model_len`` group is generate-only, while ``agg`` is the
    voting-fusion mode read by score. ``max_model_len`` is the vLLM engine context
    length (default 16384; video configs raise it, e.g. 32768). Unknown YAML keys are
    dropped when the config loads (see ``_stage``), so a stage block may set only the
    ...
    max_model_len: int = 16384  # generate: vLLM engine context length
    gpu_mem_util: float = 0.92  # generate: vLLM gpu_memory_utilization
...
def load_config(path: str) -> RunConfig:
    ...
    defaults (one rollout; an absent ``band`` means all layers, so the L8-21 middle band
    is opt-in per config), and records the file's absolute path in ``config_path`` so a
    GPU stage subprocess can rebuild the run.
    ...
    return RunConfig(
        ...
        band=raw.get(
            "band"
        ),  # absent/null = all layers; the L8-21 band is opt-in per config
