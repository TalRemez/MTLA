...
class TokenRange(TypedDict):
    ...
    Returned per prediction by an adapter's ``query_tokens`` and consumed by
...
class BuildInputs(TypedDict, total=False):
    """The per-item payload an adapter's ``build_inputs`` hands to ``compute_mtla``.
    ...
    ``build_inputs`` may instead return ``None`` to skip an item. It is
