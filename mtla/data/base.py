...
— shard loading, band reduction, voting, NMS, and metric evaluation all live in the ``evaluate.py``
stage, ``mtla.voting``, and ``mtla.metrics``. This keeps every adapter small and identical in shape.
...
def print_metrics(name: str, metrics: dict, indent: str = "  ") -> None:
    ...
    Renders the metrics mapping the ``evaluate.py`` pipeline assembles. Top-level float
...
class DatasetAdapter:
    ...
    the ``evaluate.py`` pipeline reads to route the prediction through the right
