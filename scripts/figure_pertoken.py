...
DEFAULT_TARGETS = [(64499, 0, True),       # grounded: zebra
                   (60823, 3, False)]      # hallucinated: cow→"horse"
...
def parse_targets(specs):
    """`["64499:0:grounded", "60823:3:hallu", ...]` -> [(id, pi, grounded_bool)]."""
    out = []
    for s in specs:
        iid, pi, *rest = s.split(":")
...
def find_coord_groups(response, pred_bboxes, tokenizer):
    """Per prediction, the response token indices of each coord (x1,y1,x2,y2) and the label,
    matched by the `"bbox_2d":[...],"label":"..."` span. Mirrors the paper's figure extractor."""
    import re
    enc = tokenizer(response, add_special_tokens=False, return_offsets_mapping=True)
    off = enc["offset_mapping"]
    tmpl = (r'"bbox_2d"\s*:\s*\[\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\]\s*,'
            r'\s*"label"\s*:\s*"{l}"')
    out, sp = [], 0
    for pb in pred_bboxes:
        ...
        coord_groups = [[ti for ti, (ts, te) in enumerate(off) if ts < m.end(g) and te > m.start(g)]
                        for g in range(1, 5)]
...
def build_hook(state):
    """Capture hook: stash the full per-query attention to image tokens `[Nq, L, H, n_img]`.
    Numerically identical to the stock eager forward otherwise."""
    import torch
    ...
    def hook(module, query, key, value, attention_mask, scaling, dropout=0.0, **kw):
        ...
        if s["active"] and id(module) in s["ids"]:
            ...
            rows = aw[0].index_select(1, s["qpos"]).transpose(0, 1).float()   # [Nq, H, K]
            s["buf"][:, li, :, :] = rows.index_select(2, s["imgidx"]).cpu()    # [Nq, L, H, n_img]
        out = torch.matmul(nn.functional.dropout(aw, p=dropout, training=module.training),
                           vs).transpose(1, 2).contiguous()
        return out, None
    return hook
...
def main():
    ...
    ap.add_argument("--config", required=True, help="a COCO config (for model + dataset paths)")
    ...
    ap.add_argument("--norm", choices=["per-column", "per-map"], default="per-column",
    ...
    dev = f"cuda:{args.gpu}"
    state = {"active": False, "ids": set(), "order": [], "qpos": None, "imgidx": None, "buf": None}
    ...
    net = Qwen3VLForConditionalGeneration.from_pretrained(
        model.model_id, dtype=torch.bfloat16, attn_implementation="eager", device_map=dev).eval()
    layers = net.model.language_model.layers
    nL = len(layers); nH = net.config.text_config.num_attention_heads
    ...
    for iid, pi, grounded in targets:
        if iid not in ds_by_id or iid not in preds_by_id:
            print(f"[skip] {iid}: not in dataset/predictions"); continue
        ...
        pid = inp["input_ids"][0]; plen = pid.shape[0]
        h_, w_ = inp["image_grid_thw"][0].tolist()[1:]
        gh, gw = h_ // 2, w_ // 2; nimg = gh * gw
        resp = pr["response"]
        rids = torch.tensor(proc.tokenizer(resp, add_special_tokens=False)["input_ids"],
                            dtype=pid.dtype, device=dev)
        full = torch.cat([pid, rids]).unsqueeze(0); total = full.shape[1]
        imgidx = [i for i, t in enumerate(pid.cpu().tolist()) if t == img_pad]
        if len(imgidx) != nimg:
            print(f"[skip] {iid}: image-token count {len(imgidx)} != {nimg}"); continue

        ranges = find_coord_groups(resp, pbs, proc.tokenizer)
        r = ranges[pi]
        if r is None:
            print(f"[skip] {iid} pi={pi}: could not locate the prediction's tokens"); continue
        # read attention at each token's PREDICTING position (t-1), clamped into range
        cg = [[max(0, plen + t - 1) for t in g if plen + t < total] for g in r["coord_groups"]]
        lp = [max(0, plen + t - 1) for t in r["label_toks"] if plen + t < total]
        if any(len(g) == 0 for g in cg) or not lp:
            print(f"[skip] {iid} pi={pi}: empty coord/label token group"); continue
        ...
        state["active"] = True
        with torch.no_grad():
            net(input_ids=full, pixel_values=inp["pixel_values"],
                image_grid_thw=inp["image_grid_thw"],
                attention_mask=torch.ones(1, total, device=dev, dtype=torch.long))
        state["active"] = False
        buf = state["buf"].numpy()

        def red(pos):  # [n_tok, L, H, nimg] -> [gh, gw]: mean sub-tokens, band, mean heads, sum layers
        ...
        rows.append({"iid": iid, "grounded": grounded, "box": pbs[pi]["box"],
                     "label": pbs[pi]["label"], "variants": variants,
                     "image": np.asarray(img)})
        torch.cuda.empty_cache()

    if not rows:
        print("no rows rendered; check --targets / that the COCO predictions exist"); sys.exit(1)
    ...
    if args.norm == "per-column":
        colref = [float(np.percentile(np.concatenate([r["variants"][c].ravel() for r in rows]), 99)) + 1e-9
                  for c in range(ncol)]
    else:
        colref = None

    nr = len(rows)
    fig, axes = plt.subplots(nr, 7, figsize=(7 * 1.6, nr * 1.6 * 0.85 + 0.4), squeeze=False)
    cmap = plt.get_cmap("turbo")
    for ri, r in enumerate(rows):
        col = GROUNDED_C if r["grounded"] else HALLU_C
        img = r["image"]; H, W = img.shape[:2]
        b = r["box"]; x1, y1 = b[0] * W / 1000, b[1] * H / 1000
        x2, y2 = b[2] * W / 1000, b[3] * H / 1000
        ax0 = axes[ri, 0]; ax0.imshow(img, interpolation="none")
        ax0.add_patch(Rectangle((x1, y1), x2 - x1, y2 - y1, ec=col, fc="none", lw=2.2))
        yt, va = (y1 - 2, "bottom") if y1 > 14 else (y1 + 2, "top")
        ax0.text(x1, yt, f" {r['label']} ", color="white", fontsize=7, fontweight="bold",
                 va=va, ha="left", bbox=dict(facecolor=col, edgecolor="none",
                 boxstyle="round,pad=0.12"), clip_on=False)
        ...
        for ci, gmap in enumerate(r["variants"]):
            ...
            al = np.where(hm <= lo, 0.0, viz.FLOOR + (viz.AMAX - viz.FLOOR) * tt)[..., None]
            comp = ((1 - al) * img.astype(np.float32) + al * (cmap(np.clip(hm, 0, 1))[..., :3] * 255)
                    ).clip(0, 255).astype(np.uint8)
            ax.imshow(comp, interpolation="none")
            ax.add_patch(Rectangle((x1, y1), x2 - x1, y2 - y1, ec="white", fc="none", lw=1.2))
            ax.set_xticks([]); ax.set_yticks([])
            if ri == 0:
                ax.set_title(COLS[ci + 1], fontsize=13)

    fig.subplots_adjust(left=0.055, right=0.997, top=0.94, bottom=0.01, wspace=0.03, hspace=0.06)
    ...
    def band(idx, color, text):
        ...
        fig.add_artist(FancyBboxPatch((0.012, min(bots)), 0.02, max(tops) - min(bots),
                       boxstyle="round,pad=0.002", transform=fig.transFigure,
                       facecolor=color, edgecolor="none", alpha=0.9, zorder=5))
        fig.text(0.022, (min(bots) + max(tops)) / 2, text, rotation=90, va="center", ha="center",
    ...
    fig.add_artist(plt.Line2D([xsep, xsep], [axes[nr - 1, 6].get_position().y0,
                   axes[0, 6].get_position().y1], transform=fig.transFigure,
                   color="#bbb", lw=0.8, zorder=7))
