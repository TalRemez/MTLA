...
rolls=sorted(int(d.split('rollout')[1]) for d in glob.glob('runs/coco/internvl_image/features/rollout*'))
print('rollouts:', rolls, flush=True)
recs_by_roll={}
for rk in rolls:
    rr=[]
    for f in glob.glob(f'runs/coco/internvl_image/features/rollout{rk}/shard*.pt'):
